...
- **New data (privacy-safeguarded)**: To evaluate how the tuned model generalizes to unseen, differently-sourced data, I designed a custom occupational well-being questionnaire and distributed it to real employees at the company. The questionnaire was purpose-built to produce data structurally comparable to the HackerEarth training set, so that the tuned model could ultimately be tested on responses from real employees rather than another public dataset. It is organized into four sections: **Demographics** (role, department, region, tenure, age group, ethnicity, gender, client-facing status, remote-working setup); a **Burnout Index**, based on the Maslach Burnout Inventory - General Survey (MBI-GS), covering Emotional Exhaustion, Cynicism, and Professional Efficacy; a **Mental Fatigue Score**, inspired by the Occupational Fatigue Exhaustion Recovery (OFER) scale; and eleven **Psychosocial Predictors** (e.g., Workload, Job Satisfaction, Control & Autonomy, Reward & Recognition, Financial Stability, Shared Values, Supervisor Support, Fairness, Community, Toxicity at Work, AI Familiarization), each measured through several 5-point Likert-scale items.

  Since all questionnaire items were rated on a 5-point Likert scale, the raw responses were transformed so that the resulting variables would sit on the same numerical ranges as their counterparts in the training set: the Burnout Index was rescaled to a continuous 0–1 range to match *Burn Rate*, and the Mental Fatigue Score was rescaled to a 1–10 range to match the training set's *Mental Fatigue Score*. Other demographic fields (e.g., tenure, designation, department) were likewise mapped onto categories consistent with the training data's schema.
